...
export default function JobTracker() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const jr = await window.storage.get("aluminum-glass-job-data");
        if (jr && jr.value) {
          const parsed = JSON.parse(jr.value);
      ...
      try {
        const ir = await window.storage.get("aluminum-glass-inventory-data");
        if (ir && ir.value) {
          const parsedInv = JSON.parse(ir.value);
  ...
  const persistJobs = useCallback(async (nextJobs) => {
    setSaveState("saving");
    try {
      const result = await window.storage.set("aluminum-glass-job-data", JSON.stringify({ jobs: nextJobs }));
  ...
  const persistInventory = useCallback(async (nextInv) => {
    setSaveState("saving");
    try {
      const result = await window.storage.set("aluminum-glass-inventory-data", JSON.stringify({ items: nextInv }));
      if (!result) throw new Error("no result");
      setSaveState("saved");
      setTimeout(() => setSaveState((s) => (s === "saved" ? "idle" : s)), 1500);
    } catch (e) {
